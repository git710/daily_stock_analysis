...
def test_config():
    ...
    print(
        f"  企业微信 Webhook: {'已配置 ✓' if config.wechat_webhook_url else '未配置 ✗'}"
    )
...
def view_database():
    ...
    try:
        ...
        if stocks:
            print(
                f"  {'代码':<10} {'记录数':<8} {'起始日期':<12} {'最新日期':<12} {'数据源'}"
            )
            print("  " + "-" * 60)
            for row in stocks:
                print(
                    f"  {row[0]:<10} {row[1]:<8} {row[2]!s:<12} {row[3]!s:<12} {row[4] or 'Unknown'}"
                )
        else:
            print("  暂无数据")
        ...
        if today_data:
            for row in today_data:
                ...
                print(f"\n  【{code}】")
                print(
                    f"    开盘: {open_:.2f}  最高: {high:.2f}  最低: {low:.2f}  收盘: {close:.2f}"
                )
                print(f"    涨跌幅: {pct_chg:.2f}%  成交量: {volume / 10000:.2f}万股")
                print(
                    f"    MA5: {ma5:.2f}  MA10: {ma10:.2f}  MA20: {ma20:.2f}  量比: {vol_ratio:.2f}"
                )
        else:
            print("  今日暂无数据")
        ...
        if recent:
            print(
                f"  {'代码':<10} {'日期':<12} {'收盘':<10} {'涨跌%':<8} {'成交量':<15} {'来源'}"
            )
            print("  " + "-" * 70)
            for row in recent:
...
def test_llm():
    ...
    try:
        ...
        print("  ✓ 可以连接到 Google API 服务器")
    except Exception as e:
        print(f"  ✗ 无法连接到 Google API 服务器: {e}")
        print("  提示: 请检查网络连接或配置代理")
        print("  提示: 可以设置环境变量 HTTPS_PROXY=http://your-proxy:port")
        return False
...
def test_notification():
    ...
    if service.is_available():
        ...
        webhook_preview = (
            config.wechat_webhook_url[:50] + "..."
            if len(config.wechat_webhook_url) > 50
            else config.wechat_webhook_url
        )
...
def run_all_tests():
    ...
    print("提示: 使用 --fetch 参数单独测试数据获取")
    print("提示: 使用 --notify 参数单独测试通知推送")
...
def main():
    ...
    # 如果没有指定任何参数，运行基础测试
    if not any(
        [args.db, args.llm, args.fetch, args.notify, args.config, args.stock, args.all]
    ):
        run_all_tests()
        return 0
