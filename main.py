...
def get_previous_trading_day(current_date: Optional[date] = None) -> date:
    """
    获取上一个交易日（跳过周末）

    A股交易日为周一至周五，如果当前是周一，则返回上周五
    ...
    # 跳过周末（周六和周日）
    while check_date.weekday() >= 5:  # 5=周六, 6=周日
...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            # 断点续传检查：如果目标日期数据已存在，跳过
            if not force_refresh and self.db.has_today_data(code, target_date):
                logger.info(f"[{code}] {target_date} 数据已存在，跳过获取（断点续传）")
                return True, None
            ...
            logger.info(
                f"[{code}] 数据保存成功（来源: {source_name}，新增 {saved_count} 条）"
            )
    ...
    def analyze_stock(self, code: str) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            try:
                # 获取历史数据进行趋势分析（使用上一个交易日的数据）
                previous_trading_day = get_previous_trading_day()
                context = self.db.get_analysis_context(
                    code, target_date=previous_trading_day
                )
    ...
    def _enhance_context(
        ...
        if realtime_quote:
            enhanced["realtime"] = {
                ...
                "volume_ratio_desc": self._describe_volume_ratio(
                    realtime_quote.volume_ratio
                ),
    ...
    def process_single_stock(
        ...
        try:
            # Step 1: 获取并保存数据
            # 在早上9点运行时，获取上一个交易日的数据
            current_time = datetime.now()
            is_morning_run = current_time.hour < 12  # 早上运行（9点左右）

            if is_morning_run:
                # 早上9点运行，分析上一个交易日的数据，确保数据存在
                previous_trading_day = get_previous_trading_day()
                success, error = self.fetch_and_save_stock_data(
                    code, target_date=previous_trading_day
                )
                logger.info(
                    f"[{code}] 早上运行模式：确保 {previous_trading_day} 数据存在"
                )
            else:
                # 下午运行，获取今日数据
...
def run_market_review(
    notifier: NotificationService, analyzer=None, search_service=None
) -> Optional[str]:
    """
    执行大盘复盘分析
    ...
    try:
        market_analyzer = MarketAnalyzer(
            search_service=search_service, analyzer=analyzer
        )
        ...
        if args.market_review:
            ...
            if config.tavily_api_keys or config.serpapi_keys:
                search_service = SearchService(
                    tavily_keys=config.tavily_api_keys, serpapi_keys=config.serpapi_keys
                )
