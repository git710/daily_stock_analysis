...
class NotificationService:
    ...
    def generate_daily_report(
        ...
        # 统计信息
        buy_count = sum(
            1 for r in results if r.operation_advice in ["买入", "加仓", "强烈买入"]
        )
        sell_count = sum(
            1 for r in results if r.operation_advice in ["卖出", "减仓", "强烈卖出"]
        )
        hold_count = sum(1 for r in results if r.operation_advice in ["持有", "观望"])
        # 统计异常项（不属于上述三类的）
        other_count = len(results) - buy_count - sell_count - hold_count
        if other_count > 0:
            logger.warning(f"发现 {other_count} 只股票的操作建议未分类")
        avg_score = (
        ...
        for result in sorted_results:
            ...
            # 判断是否为 ETF，添加标签
            etf_tag = " [ETF]" if hasattr(result, "is_etf") and result.is_etf else ""

            report_lines.extend(
                [
                    f"### {emoji} {result.name} ({result.code}){etf_tag}",
                    "",
    ...
    def generate_dashboard_report(
        ...
        for result in sorted_results:
            ...
            report_lines.extend(
                [
                    f"## {signal_emoji} {stock_name} ({result.code}){etf_tag}",
                    "",
                ]
            )
            ...
            if intel:
                ...
                if intel.get("sentiment_summary"):
                    report_lines.append(
                        f"**💭 舆情情绪**: {intel['sentiment_summary']}"
                    )
            ...
            if data_persp:
                ...
                if trend_data:
                    is_bullish = (
                        "✅ 是" if trend_data.get("is_bullish", False) else "❌ 否"
                    )
                    report_lines.extend(
                        [
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        for result in sorted_results:
            ...
            # 标题行：信号等级 + 股票名称
            lines.append(
                f"### {signal_emoji} **{signal_text}** | {stock_name}({result.code}){etf_tag}"
            )
            lines.append("")
            ...
            if checklist:
                # 只显示不通过的项目
                failed_checks = [
                    c for c in checklist if c.startswith("❌") or c.startswith("⚠️")
                ]
                if failed_checks:
                    lines.append("**检查未通过项**:")
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        for result in sorted_results:
            ...
            # 核心信息行
            lines.append(f"### {emoji} {result.name}({result.code}){etf_tag}")
            lines.append(
        ...
        try:
            ...
            if len(content) <= max_length:
                ...
                return self._send_telegram_chunked(
                    api_url, chat_id, content, max_length
                )
    ...
    def _send_telegram_chunked(
        self, api_url: str, chat_id: str, content: str, max_length: int
    ) -> bool:
        """分段发送长 Telegram 消息"""
        # 按段落分割
        sections = content.split("\n---\n")
    ...
    def send_to_custom(self, content: str) -> bool:
        ...
        for i, url in enumerate(self._custom_webhook_urls):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"自定义 Webhook {i + 1} 推送失败: HTTP {response.status_code}"
                    )
        ...
        logger.info(
            f"自定义 Webhook 推送完成：成功 {success_count}/{len(self._custom_webhook_urls)}"
        )
    ...
    def send(self, content: str) -> bool:
        ...
        logger.info(
            f"正在向 {len(self._available_channels)} 个渠道发送通知：{channel_names}"
        )
